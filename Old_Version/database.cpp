...
bool database::findCustomer(int ID)
{
...
bool database::transferMoney(int forwarder, int recipient, double amount)
{
    ...
    if(m_balance>amount || m_balance==amount){
        ...
        if(m_bank!=bank){
            ...
            info=QString::number(processingFee)+" TL iSLEM UCRETi TAHSiL EDiLMiSTiR.";
        }
        else
            info="BANKANIZ BU iSLEM iCiN SiZDEN UCRET TAHSiL ETMEMiSTiR.";
